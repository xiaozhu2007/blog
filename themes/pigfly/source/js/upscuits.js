...
myApp.dashboard = (function ($) {
  ...
  function getUptime(apikey, ids) {
    var url =
      ...
      apikey +
      "&customUptimeRatio=1-2-3-4-5-6-7-30&format=json&logs=1&noJsonCallback=1";
    $.ajax({
      url: url,
  ...
  function placeServer(data, ids) {
    ...
    while ((st = fin.pop())) {
      ...
      if (st.len == 1) {
        ...
        if (st.right - st.left < 1000 * 3540) {
          temptip +=
            " (" +
            new Number((st.right - st.left) / (1000 * 60)).toFixed(0) +
            " 分钟)";
        } else {
          temptip +=
            " (" +
            new Number((st.right - st.left) / (1000 * 3600)).toFixed(1) +
            " 小时)";
        }
        temptip +=
          '<br><span class="ttime">' +
    ...
    for (a = 0; a < uptimes.length; a++) {
      tm = (100 - uptimes[a]) * (a == uptimes.length - 1 ? 14.4 * 30 : 14.4);
      th = tm / 60;
      if (uptimes[a] >= 99.97) {
        uptimeb[a] = "可用率 100%";
      } else if (uptimes[a] <= 0) {
        uptimeb[a] =
          "可用率 0.00%故障 " +
          (a == uptimes.length - 1 ? "720 小时" : "24 小时");
      } else if (tm < 60) {
        uptimeb[a] =
          "可用率 " +
          new Number(uptimes[a]).toFixed(2) +
          "%<br>故障 " +
          new Number(tm).toFixed(0) +
          " 分钟";
      } else {
        uptimeb[a] =
          "可用率 " +
  ...
  function Type2Word(t, icon) {
    switch (t) {
      ...
      case 2:
        return (
          (icon ? '<span class="fa fa-check-circle"></span> ' : "") + "正常"
        );
      //case 99:
      //	return "未知";
      //case 98:
      ...
      default:
        return (
          (icon ? '<span class="fa fa-times-circle"></span> ' : "") + "未知"
        );
    }
  }
